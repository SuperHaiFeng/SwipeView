...
- (void)updateItemSizeAndCount
{
    ...
    {
		UIView *view = [[self visibleItemViews] lastObject];
		if (!view) {
			if ([_dataSource respondsToSelector:@selector(swipeView:tempViewForItemAtIndex:reusingView:)]) {
				view = [_dataSource swipeView:self tempViewForItemAtIndex:0 reusingView:[self dequeueItemView]];
			} else {
				view = [_dataSource swipeView:self viewForItemAtIndex:0 reusingView:[self dequeueItemView]];
			}
		}
